...
## Docker 构建
- Dockerfile 多阶段构建：
  - [Dockerfile](file:///d:/aiback/proj/backend/ai-interview-controller/Dockerfile)
  - 产物：`ruoyi-admin/target/ruoyi-admin.jar` → `eclipse-temurin:17-jre-alpine`
- 手动构建镜像：
  - `docker build -t ai-interview-backend -f backend/ai-interview-controller/Dockerfile backend/ai-interview-controller`

## Docker Compose 部署
- 环境文件：
  - [deployment/.env](file:///d:/aiback/proj/backend/ai-interview-controller/deployment/.env)（包含 `DB_USERNAME/DB_PASSWORD`、MinIO、AI 等）
- 核心服务：
  - ai-interview-backend：后端容器，端口映射 `8081:8080`
  - Redis/MinIO：通过网络与前端 Nginx 互通（Redis 主机名 `redis`，MinIO 通过前端反代 `/pic/`）
- 启动：
  - `docker compose -f deployment/docker-compose.yml up -d --build`

## 反向代理与匿名访问
- 前端 Nginx 反代后端接口：
  - `/prod-api/` → `ai-interview-backend:8080/`（保留末尾斜杠以去前缀）
  - `/pic/` → `minio:9000/`（MinIO S3 API）
- 后端匿名白名单：
  - 见 [SecurityConfig](file:///d:/aiback/proj/backend/ai-interview-controller/ruoyi-framework/src/main/java/com/ruoyi/framework/config/SecurityConfig.java#L96-L118)
  - `"/login", "/register", "/captchaImage", "/ws/**", "/mp/**", "/ai/**"` 等

## 常见问题
- 连接数据库失败（Communications link failure）：
  - 容器访问宿主机 MySQL：使用 `host.docker.internal:3306` 并保证 MySQL 监听非 127.0.0.1
- 认证失败（using password: NO）：
  - 确保 compose 中 `DB_USERNAME/DB_PASSWORD` 设置，并未被空值覆盖；或直接在 `application-druid.yml` 写入账号密码
- 验证码 401：
  - 代理未去除 `/prod-api` 前缀导致；确保 `location /prod-api/ { proxy_pass http://.../; }`
- MinIO 图片无法访问：
  - Nginx 上游必须指向 `minio:9000`（容器内端口），不是宿主机映射 `19000`
